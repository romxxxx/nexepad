...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of nexepad instances
	nexepad1, nexepad2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := nexepad1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In nexepad2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, nexepad2)
	}

	// Connect the two nexepads. This should trigger sync
	// between the two nodes
	connect(t, nexepad1, nexepad2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := nexepad1.rpcClient.GetVirtualSelectedParentChainFromBlock(
