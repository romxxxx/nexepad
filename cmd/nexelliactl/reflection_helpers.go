...
// 2. A wrapper that implements isnexepadMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `nexepadMessage_` prefix and `Request` suffix
...
// generatenexepadMessage generates a wrapped nexepadMessage with the given `commandValue`
func generatenexepadMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.nexepadMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	nexepadMessage := reflect.New(reflect.TypeOf(protowire.nexepadMessage{}))
	nexepadMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return nexepadMessage.Interface().(*protowire.nexepadMessage), nil
}
