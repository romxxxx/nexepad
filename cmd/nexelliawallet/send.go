...
func send(conf *sendConfig) error {
	...
	if !conf.IsSendAll {
		sendAmountSompi = uint64(conf.SendAmount * constants.SompiPerNexellia)
	}
	...
	for i, unsignedTransaction := range createUnsignedTransactionsResponse.UnsignedTransactions {
		signedTransaction, err := libnexelliawallet.Sign(conf.NetParams(), mnemonics, unsignedTransaction, keysFile.ECDSA)
		if err != nil {
			return err
		}
