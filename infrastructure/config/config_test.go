...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-nexepad.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "nexepad")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of nexepad binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-nexepad.conf")
