...
const (
	defaultConfigFilename      = "nexepad.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "nexepad.log"
	defaultErrLogFilename      = "nexepad_err.log"
	...
	sampleConfigFilename    = "sample-nexepad.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for nexepad.
	DefaultAppDir = util.AppDir("nexepad", false)
...
// Flags defines the configuration options for nexepad.
//
...
type Flags struct {
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in nexe/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for nexepad.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default nexepad configuration
...
//
// The above results in nexepad functioning properly without any config settings
...
// createDefaultConfig copies the file sample-nexepad.conf to the given destination path,
