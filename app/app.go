...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "nexepadsvc",
	DisplayName: "nexepad Service",
	Description: "Downloads and stays synchronized with the nexellia blockDAG and " +
		"provides DAG services to applications.",
}

type nexepadApp struct {
	cfg *config.Config
}

// StartApp starts the nexepad app, and blocks until it finishes running
func StartApp() error {
...
func (app *nexepadApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start nexepad: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down nexepad...")
		...
		log.Infof("nexepad shutdown complete")
	}()
