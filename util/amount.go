...
// other than the base unit of a Nexellia. The value of the AmountUnit
// is the exponent component of the decadic multiple to convert from
// an amount in Nexellia to an amount counted in units.
type AmountUnit int

// These constants define various units used when describing a Nexellia
// monetary amount.
const (
...
// Amount represents the base Nexellia monetary unit (colloquially referred
// to as a `Sompi'). A single Amount is equal to 1e-8 of a Nexellia.
type Amount uint64
...
// some value in Nexellia. NewAmount errors if f is NaN or +-Infinity, but
// does not check that the amount is within the total amount of Nexellia
...
func NewAmount(f float64) (Amount, error) {
	...
	case math.IsInf(f, 1):
		fallthrough
	case math.IsInf(f, -1):
		return 0, errors.New("invalid Nexellia amount")
	}

	return round(f * constants.SompiPerNexellia), nil
}

// ToUnit converts a monetary amount counted in Nexellia base units to a
// floating point value representing an amount of Nexellia.
...
// Format formats a monetary amount counted in Nexellia base units as a
...
// useful for services that build on top of Nexellia (for example, calculating
