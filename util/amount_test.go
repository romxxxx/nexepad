...
func TestAmountUnitConversions(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			s:         "444333.222111 nexe",
		},
		{
			name:      "mnexe",
			amount:    44433322211100,
			unit:      AmountMillinexe,
			converted: 444333222.11100,
			s:         "444333222.111 mnexe",
		},
		{

			name:      "μnexe",
			amount:    44433322211100,
			unit:      AmountMicronexe,
			converted: 444333222111.00,
			s:         "444333222111 μnexe",
		},
		{
	...
	for _, test := range tests {
		...
		// Verify that Amount.Tonexe works as advertised.
		f1 := test.amount.ToUnit(Amountnexe)
		f2 := test.amount.Tonexe()
		if f1 != f2 {
			t.Errorf("%v: Tonexe does not match ToUnit(Amountnexe): %v != %v", test.name, f1, f2)
		}

		// Verify that Amount.String works as advertised.
		s1 := test.amount.Format(Amountnexe)
		s2 := test.amount.String()
		if s1 != s2 {
			t.Errorf("%v: String does not match Format(Amountnexe): %v != %v", test.name, s1, s2)
		}
	}
}
...
func TestAmountMulF64(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Multiply 0.1 nexe by 2",
			amt:  100e5, // 0.1 nexe
			mul:  2,
			res:  200e5, // 0.2 nexe
		},
		{
			name: "Multiply 0.2 nexe by 0.02",
			amt:  200e5, // 0.2 nexe
			mul:  1.02,
			res:  204e5, // 0.204 nexe
		},
		{
			name: "Round down",
			amt:  49, // 49 Sompi
